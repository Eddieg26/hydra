...
pub enum NativeKey {
    ...
    /// An XKB "keysym".
    Xkb(u32),
    /// A web key identifier, which is a string that identifies a key in a way that is consistent across browsers.
    Web(SmolStr),
}
...
pub struct KeyboardInput {
    ...
    /// This is a higher-level representation of the key, which may include modifiers and other
    /// contextual information.
    /// It is used to identify the key in a way that is consistent across different platforms.
    pub key: Key,

    /// The text that was generated by the key event, if any.
    /// This is typically the character that the key produces when pressed.
    /// This may be `None` if the key cannot produce text (e.g., modifier keys).
    pub text: Option<SmolStr>,

    /// Whether the key event is a repeat event.
    /// This is `true` if the key is being held down and the system is generating repeated events for it.
    pub repeat: bool,
}

impl KeyboardInput {
    pub(crate) fn process_events(
        key_codes: &mut Buttons<KeyCode>,
        keys: &mut Buttons<Key>,
        events: EventReader<KeyboardInput>,
    ) {
        for event in events {
            let KeyboardInput {
                code, state, key, ..
            ...
            key_codes.set(*code, *state);
            keys.set(key.clone(), *state);
        }
    }
}
