use super::{AsyncIoError, AsyncReader, AsyncWriter, FileSystem, PathStream};
use crate::{ext::PathExt, io::BoxFuture};
use smol::{fs::File, io::AsyncReadExt, stream::StreamExt};
use std::path::Path;
...
impl AsyncWriter for File {
    fn flush<'a>(&'a mut self) -> BoxFuture<'a, Result<(), AsyncIoError>> {
        Box::pin(async move {
            smol::io::AsyncWriteExt::flush(self)
                .await
...
impl FileSystem for LocalFs {
    ...
    async fn reader(&self, path: &std::path::Path) -> Result<Self::Reader, AsyncIoError> {
    ...
    async fn read_dir(&self, path: &std::path::Path) -> Result<Box<dyn PathStream>, AsyncIoError> {
        let path = path.with_prefix(&self.root);
    ...
    async fn is_dir(&self, path: &std::path::Path) -> Result<bool, AsyncIoError> {
        let path = path.with_prefix(&self.root);
    ...
    async fn writer(&self, path: &std::path::Path) -> Result<File, AsyncIoError> {
        let path = path.with_prefix(&self.root);
    ...
    async fn create_dir(&self, path: &Path) -> Result<(), AsyncIoError> {
        let path = path.with_prefix(&self.root);
    ...
    async fn create_dir_all(&self, path: &Path) -> Result<(), AsyncIoError> {
        let path = path.with_prefix(&self.root);
    ...
    async fn remove(&self, path: &std::path::Path) -> Result<(), AsyncIoError> {
        let path = path.with_prefix(&self.root);
    ...
    async fn remove_dir(&self, path: &std::path::Path) -> Result<(), AsyncIoError> {
        let path = path.with_prefix(&self.root);
