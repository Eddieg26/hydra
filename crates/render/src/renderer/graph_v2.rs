use ecs::{Entity, IndexMap, SystemMeta, World};
use std::{
    any::Any,
    cell::UnsafeCell,
...
impl<R: GraphResource> GraphPass for CreateResourcePass<R> {
    ...
    fn setup(
        ...
        move |ctx| Ok(ctx.graph.get_mut().resources[id].create(ctx.world, ctx.device, ctx.surface))
    }
}

pub struct RenderContext<'a> {
    view: Option<Entity>,
    graph: UnsafeCell<&'a mut RenderGraph>,
    world: &'a World,
...
impl<'a> RenderContext<'a> {
    ...
    pub fn get<R: GraphResource>(&self, id: GraphResourceId<R>) -> Option<&R> {
        unsafe { self.graph.get().as_ref().unwrap_unchecked().get::<R>(id) }
    }
    ...
    fn run_sub_graph(&mut self, name: Name) -> Result<(), RenderGraphError> {
        let graph = unsafe { self.graph.get().as_ref().unwrap() };
        if let Some(passes) = graph.sub_graphs.get(&GraphId::Sub(name)) {
            for pass in passes {
                pass.execute(self)?;
            }
        }
...
pub struct RenderGraph {
    resources: IndexMap<Name, ResourceNode>,
    passes: Vec<PassNode>,
    sub_graphs: HashMap<GraphId, Vec<PassNode>>,
}
