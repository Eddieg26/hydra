use crate::{
    ...
    PipelineId, RenderAssets, RenderPipelineDesc, RenderResource, RenderState, RenderSurface,
    Shader, SubMesh, VertexState,
    allocator::MeshAllocator,
    cpu::ModelUnifomBuffer,
    ...
    pub(crate) fn extract(
        ...
        for (entity, draw, local, global, filter) in query.iter() {
            extracted.push(DrawInstance {
                ...
                draw: draw.clone(),
                local: *local,
                model: ModelTransform::from(global.matrix()),
            });
        }
...
#[derive(Resource)]
pub struct PhaseDrawCalls<P: RenderPhase> {
    pub(super) views: HashMap<Entity, Vec<DrawCall<P>>>,
}
impl<P: RenderPhase> PhaseDrawCalls<P> {
    pub fn new() -> Self {
        Self {
            views: HashMap::new(),
        }
        ...
        D::Material: Material<Phase = P>,
    {
        ...
        let views = world.resource::<ViewBuffer<P::View>>();
        ...
        match &call.mode {
            DrawMode::Direct {
                ...
                let models = world.resource::<ModelUnifomBuffer>();
            ...
            DrawMode::Indirect { offset } => {
                // let instances = world.resource::<StorageDataBuffer<ModelTransform>>();
                ...
                unimplemented!("Gpu driven rendering not implemented yet")
            }
