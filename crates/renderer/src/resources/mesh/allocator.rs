use crate::{
    core::RenderDevice,
    resources::{Buffer, BufferDesc, GpuMesh, Mesh, MeshFormat, MeshVertexInfo},
};
use asset::AssetId;
use ecs::Resource;
...
pub struct MeshAllocator {
    ...
    /// A map of chunk IDs to pending allocations that need to be written to the GPU.
...
impl MeshAllocator {
    ...
    pub fn allocate(
        ...
        let meta = ChunkMeta::new(ChunkUsage::Vertex, size as u32);
        self.queue(device, id, vertices, meta)?;

        let format = if let Some(indices) = mesh.indices() {
            let meta = ChunkMeta::new(ChunkUsage::Index, indices.format().byte_size() as u32);
            let data = indices.data().to_vec();
            self.queue(device, id, data, meta)?;
            MeshFormat::Indexed {
        ...
        if alloc_size < self.config.block_threshold {
            ...
            if let Some((chunk, allocation)) = result {
                ...
                if let Some(allocation) = chunk.allocate(*id, alloc_size as usize) {
                    ...
                    self.pending
                        .insert(chunk_id, PendingAllocations::with(*id, allocation, data));
    ...
    fn update(&mut self, device: &RenderDevice) {
        for (chunk_id, pending) in self.pending.drain() {
    ...
    pub fn release(&mut self, id: &AssetId<Mesh>) {
    ...
    pub(crate) fn commit(allocator: &mut Self, device: &RenderDevice) {
        allocator.update(device);
    }
}
